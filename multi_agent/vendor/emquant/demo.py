...
try:
    ...
    #cmc支持批量了，为保持原有单个代码结构不变，原先单个请求不做变动传入mode=batch强制使用新返回结构，批量代码直接使用新返回结构，类似csd的返回结构
	#data = c.cmc("300059.SZ", "OPEN,CLOSE,HIGH", (datetime.today() + timedelta(-6)).strftime("%Y-%m-%d"), datetime.today().strftime("%Y-%m-%d"), "mode=batch,RowIndex=2,Ispandas=0")
    data = c.cmc("300059.SZ,300061.SZ", "OPEN,CLOSE,HIGH", (datetime.today() + timedelta(-6)).strftime("%Y-%m-%d"), datetime.today().strftime("%Y-%m-%d"), "RowIndex=2,Ispandas=0")
    print("cmc批量输出结果======分隔线======")
    if not isinstance(data, c.EmQuantData):
        print(data)
    else:
            ...
            for code in data.Codes:
                for i, indicator in enumerate(data.Indicators):
                    for j, date in enumerate(data.Dates):
                        value = data.Data[code][i][j]
                        print(f"{code} {date} {indicator}: {value}")
    ...
    orderdict = {'code': ['300059.SZ', '600000.SH'],
                 ...
                 'destvolume': [0, 0],
                 'weight': [0.1, 0.1]}
    data = c.porder("quant001", orderdict, "测试下单")
    if(data.ErrorCode != 0):
